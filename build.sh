...
lede(){ render "$1" | sed '0,/<p>/s//<p class="lede">/'; }
...
for placeholder in $PLACEHOLDERS; do token=${placeholder#{{};token=${token%}};mode=${token%%:*};key=${token#*:};[ "$mode" != "$token" ] || { mode=render; key=$token; };slug=$(slugify "$key");[ -s "$TMP/$slug.md" ] || { echo "tiny: missing section for $placeholder" >&2; exit 1; };out="$TMP/value.$mode.$slug";case $mode in render) render "$slug" > "$out";; lede|intro) lede "$slug" > "$out";; plain|text) plain "$slug" > "$out";; raw|markdown) raw "$slug" > "$out";; *) echo "tiny: unknown mode '$mode'" >&2; exit 1;; esac;printf '%s\t%s\n' "$placeholder" "$out" >> "$MAP";done
